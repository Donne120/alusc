...
export const ChatMessage = ({
  ...
  return <div className={cn("py-6 px-4 md:px-8 w-full animate-message-fade-in relative backdrop-blur-sm", isAi ? "bg-[#1A1F2C]/50" : "bg-[#1A1F2C]/30")} id={`message-${message.slice(0, 10)}`}>
      <div className="max-w-3xl mx-auto flex gap-4">
        {isAi ? <div className="w-10 h-10 rounded-xl overflow-hidden flex-shrink-0 bg-gradient-to-tr from-[#9b87f5] to-[#8B5CF6] p-0.5">
          ...
          </div> : <div className="w-10 h-10 rounded-xl bg-gradient-to-r from-[#D946EF] to-[#8B5CF6] flex items-center justify-center flex-shrink-0 p-0.5">
        ...
        <div className="flex-1 space-y-2">
          <div className="">
            {isEditing ? <textarea value={editedMessage} onChange={e => setEditedMessage(e.target.value)} className="w-full bg-[#1A1F2C] text-white rounded-lg p-3 min-h-[100px] border border-[#9b87f5]/20" /> : cardData ? <ChatCard {...cardData} /> : <ReactMarkdown remarkPlugins={[remarkMath, remarkGfm]} rehypePlugins={[rehypeKatex]} components={{
            code: ({
              node,
              inline,
              className,
              ...
              return !inline && match ? <div className="relative group my-4">
                  ...
                  borderRadius: '0.5rem'
                        ...
                        <button onClick={handleCopy} className="absolute top-2 right-2 p-2 rounded-lg bg-[#2A2F3C] opacity-0 group-hover:opacity-100 transition-opacity">
                          {copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                        </button>
                      </div> : <code {...props} className={cn(className, "bg-[#2A2F3C] px-1.5 py-0.5 rounded-md")}>
                        {children}
                      </code>;
            }
          }}>
        ...
        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          {!isAi && <button onClick={handleEdit} className="p-2 rounded-lg hover:bg-[#2A2F3C] text-gray-400 hover:text-white transition-colors">
      ...
      {attachments && attachments.length > 0 && <div className="mt-6 grid grid-cols-2 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
          {attachments.map((attachment, index) => attachment && attachment.type === 'image' ? <div key={index} className="relative group">
                ...
                <img src={attachment.url} alt={attachment.name} className="relative rounded-lg max-h-64 object-cover w-full border border-[#9b87f5]/10" />
              </div> : attachment ? <a key={index} href={attachment.url} download={attachment.name} className="relative group">
                <div className="absolute -inset-0.5 bg-gradient-to-r from-[#9b87f5] to-[#D946EF] rounded-lg blur opacity-20 group-hover:opacity-30 transition-opacity"></div>
